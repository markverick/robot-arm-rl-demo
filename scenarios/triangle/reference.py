...
class TriangleReference:
    ...
    def eval(self):
        ...
        u = (phi / (2.0 * np.pi)) * 3.0
        u_wrapped = u % 3.0
        seg = int(np.floor(u_wrapped))
        tau = u_wrapped - seg

        p0 = self.vertices[seg]
        p1 = self.vertices[(seg + 1) % 3]

        p_local = (1.0 - tau) * p0 + tau * p1

        # dp/dphi = dp/du * du/dphi ; du/dphi = 3 / (2pi)
        dp_du = (p1 - p0)
        dp_dphi = dp_du * (3.0 / (2.0 * np.pi))

        p_d = self.pc + self.Rp @ p_local
        v_d = self.Rp @ (dp_dphi * phi_dot)
